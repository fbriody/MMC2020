...
## Data Files
### Raw
The original raw data is available on the ISBE Report Card Data Library web page  [https://www.isbe.net/Pages/Illinois-State-Report-Card-Data.aspx]. You will need to download both the fixed with data file (rc17.txt 2.4MB becomes 35.4MB) and the variable definitions (RC17_layout.xlsx). Use and/or modify import_rc17.txt [https://github.com/fbriody/MMC2020] to get a subset of the data into RStudio. 

### Six County Subset
A six county subset (2.7MB) is available [https://github.com/fbriody/MMC2020]. This file is ready to be used in R, Fathom or Excel. 
...
sixco <- rc17 %>% 
  filter(COUNTY %in% c("Cook", "Lake", "Will", "Kane", "McHenry", "Dupage"))
```
Exporting the six county subset
```
...
### Single Values (MAYBE MOVE TO DATA IMPORT SECTION)
#### Finding a School
```{r}
rc17 %>% 
