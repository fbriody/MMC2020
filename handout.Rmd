...
date: "2/1/2020"
author: "Frank Briody | Prospect High School | frankbriody@gmail.com"
fontsize: 10pt
...
2. **Easy Method**
A reasonably sized (2.7MB) data file containing 316 variables for the 2,049 schools in the Chicagoland six county region is avaialble at [http://frankbriody.com/rc17_data.zip]. The six counties are Cook, Dupage, Kane, Lake, McHenry and Will. This data file can be imported into RStudio, Fathom, Excel, etc. 

2. **Some Variables for All Schools** 
I used the import script ```import_rc17.txt``` to select 316 variables for all 3,796 Illinois schools. A 5MB data file is available at [http://frankbriody.com/rc17_data.zip]. Again, this data file can be imported into RStudio, Fathom, Excel, etc. 
...
The resulting .csv can be imported by another software platform. (Note there is no need to export if you are staying in Studio. Just refer to your new dataframe subset, in this case `sixco`.) You can customize the above command(s) to suit your needs. 

To get a .csvfile into R, either use the File menu and Import Dataset, or send the command 
```
...
First, import one of the data files into Fathom by choosing 

csv import attribute names

Second, to create summaries, tables or displays, 
...
Putting summary numbers on a plot:
...
Notice that residaul plots are residuals vs. predicted and not residauls vs. explanatory. 
...
## Random Selection
Quickly generating a sampling distribution:
```{r}
...
A function that samples then calculates confidence interval bounds and stores results in a matrix.
```{r}
rand_samp <- function(samples, vari, samp_size) {
  ...
  a[k,] <- c(v1, v2, v3, v4, v5, v6, v7)}
  colnames(a) <- c("Sample", "Mean", "StdDev", "L_Bound", "U_Bound", "Sample_Size", "Parameter")
  return(a)
  }
```

Converting the matrix to a table of output.
```{r}
